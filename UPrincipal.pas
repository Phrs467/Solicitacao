...
begin
  ...
  EditId.Enabled := True;
  EditUsuário.Enabled := True;

  if (EditUsuário.Text <> '') or (EditDescricao.Text <> '') or (MemoSolicitacao.Text <> '') then
  begin
  ...
  EditUsuário.Text := DmLogin.Usuario_Logado;
end;
...
begin
  ...
  begin
    Application.MessageBox('Você deve preencher o Usuário' , 'Aviso', MB_ICONEXCLAMATION+MB_OK);
    Abort
  end;
  ...
  begin
    Application.MessageBox('Você deve preencher a Soliticao', 'Aviso', MB_ICONEXCLAMATION+MB_OK);
    abort
  end;
